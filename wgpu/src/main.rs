...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
  ...
  let window = winit::window::WindowBuilder::new()
    .with_title("Read Clipboard Aloud")
  ...
  event_loop.run(move |event, _, control| match event {
    Event::WindowEvent { event, .. } => match event {
      WindowEvent::Resized(new_size) => model.resize(new_size),
      WindowEvent::ScaleFactorChanged { new_inner_size, .. } => {
        model.resize(*new_inner_size)
      }
...
fn write_crosshair_instance_buffer(
  ...
  let x_thickness_scale_mat = cgmath::Matrix4::from_nonuniform_scale(
    1.0,
    crosshair_thickness / height,
    1.0,
  );

  let y_thickness_scale_mat = cgmath::Matrix4::from_nonuniform_scale(
    crosshair_thickness / width,
    1.0,
    1.0,
  );

  let x_translation_mat =
    cgmath::Matrix4::from_translation([1.0, 0.0, 0.0].into());

  let left_x_translation_mat = cgmath::Matrix4::from_translation(
    [-1.0, 1.0 - 2.0 * y / height, 0.0].into(),
  );

  let left_width_scale_mat = cgmath::Matrix4::from_nonuniform_scale(
    (x - crosshair_gap) / width,
    1.0,
    1.0,
  );
  ...
  let right_width_scale_mat = cgmath::Matrix4::from_nonuniform_scale(
    (width - x - crosshair_gap) / width,
    1.0,
    1.0,
  );

  let right_x_translation_mat = cgmath::Matrix4::from_translation(
    [-1.0, -(1.0 - 2.0 * y / height), 0.0].into(),
  );
  ...
  let instances =
    [left_instance, right_instance, top_instance, bottom_instance];
